...
class Population:
    def __init__(self, IndClass: Type[Individual], dim, nb_inds_tasks: list[int], list_tasks: list[AbstractTask] = [],
        ...
        if evaluate_initial_skillFactor:
            ...
            self.ls_subPop: list[SubPopulation] = [
                SubPopulation(IndClass, skf, self.dim_uss, nb_inds_tasks[skf], list_tasks[skf]) for skf in
                range(self.nb_tasks)
            ]
    ...
    def __getIndsTask__(self, idx_task, size: int = None, replace: bool = False, type: str = 'random',
                        tournament_size=2,
                        p_ontop=None,
                        *args, **kwargs):
        '''
        ...
        if size == 0:
            ...
            return [self.ls_subPop[idx_task].ls_inds[idx] for idx in
                    idx_inds[np.random.choice(len(idx_inds), size=size, replace=replace)].tolist()]
        else:
